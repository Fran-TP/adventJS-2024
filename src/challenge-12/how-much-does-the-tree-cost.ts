function calculatePrice(ornaments: string) {
  const treeComponentPrices: Record<string, number> = {
    '*': 1,
    o: 5,
    '^': 10,
    '#': 50,
  ...
  for (let i = 0; i < ornaments.length; i++) {
    const currPrice = treeComponentPrices[ornaments[i]]
    const nextPrice = treeComponentPrices[ornaments[i + 1]]

    totalCost = [totalCost + currPrice, totalCost - currPrice][
      +(currPrice < nextPrice)
    ]
  }

  return Number.isNaN(totalCost) ? undefined : totalCost
}
